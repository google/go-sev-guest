...
func (*FreeBSDDevice) Open(_ string) error {
	return fmt.Errorf("FreeBSD is unsupported")
}

// OpenDevice fails on FreeBSD.
func OpenDevice() (*FreeBSDDevice, error) {
	return nil, fmt.Errorf("FreeBSD is unsupported")
}
...
func (*FreeBSDDevice) Ioctl(_ uintptr, _ any) (uintptr, error) {
	return 0, fmt.Errorf("FreeBSD is unsupported")
}

// Product is not supported on FreeBSD.
func (*FreeBSDDevice) Product() *spb.SevProduct {
	return &spb.SevProduct{}
}

// FreeBSDQuoteProvider implements the QuoteProvider interface with Linux's configfs-tsm.
type FreeBSDQuoteProvider struct{}

// IsSupported checks if the quote provider is supported.
func (*FreeBSDQuoteProvider) IsSupported() bool {
	return false
}

// GetRawQuote returns byte format attestation plus certificate table via ConfigFS.
func (*FreeBSDQuoteProvider) GetRawQuote(reportData [64]byte) ([]byte, error) {
	return nil, fmt.Errorf("FreeBSD is unsupported")
}

// GetRawQuoteAtLevel returns byte format attestation plus certificate table via ConfigFS.
func (*FreeBSDQuoteProvider) GetRawQuoteAtLevel(reportData [64]byte, level uint) ([]byte, error) {
	return nil, fmt.Errorf("FreeBSD is unsupported")
}
