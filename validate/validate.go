...
type Options struct {
	...
	Measurement []byte
	// ChipID is the expected CHIP_ID field. Must be nil, abi.ChipIDStruct1Size bytes (Turin), or abi.ChipIDReportSize bytes. Not checked if nil.
	ChipID []byte
...
func checkOptionsLengths(opts *Options) error {
	var chipIDErr error
	if len(opts.ChipID) != 0 && len(opts.ChipID) != abi.ChipIDStruct1Size && len(opts.ChipID) != abi.ChipIDReportSize {
		chipIDErr = fmt.Errorf("option %q length is %d. Want %d or %d", "chip_id", len(opts.ChipID), abi.ChipIDStruct1Size, abi.ChipIDReportSize)
	}
...
// On Turin, the report buffer contains the 8-byte silicon ID in its prefix.
func validateChipID(product spb.SevProduct_SevProductName, reportChipID, expectedChipID []byte) error {
	if len(expectedChipID) == 0 {
		return nil
	}
	if len(expectedChipID) == abi.ChipIDReportSize {
		if len(reportChipID) != abi.ChipIDReportSize {
			return fmt.Errorf("report field CHIP_ID has size %d, want %d", len(reportChipID), abi.ChipIDReportSize)
		}
	...
	if len(expectedChipID) == abi.ChipIDStruct1Size {
		switch product {
		case spb.SevProduct_SEV_PRODUCT_TURIN:
			if len(reportChipID) < abi.ChipIDStruct1Size {
				return fmt.Errorf("report field CHIP_ID has size %d, want at least %d", len(reportChipID), abi.ChipIDStruct1Size)
			}
			if !bytes.Equal(expectedChipID, reportChipID[:abi.ChipIDStruct1Size]) {
				return fmt.Errorf("report field CHIP_ID is %s. Expect %s",
					hex.EncodeToString(reportChipID[:abi.ChipIDStruct1Size]), hex.EncodeToString(expectedChipID))
			}
			return nil
		case spb.SevProduct_SEV_PRODUCT_UNKNOWN, spb.SevProduct_SEV_PRODUCT_MILAN, spb.SevProduct_SEV_PRODUCT_GENOA:
			return fmt.Errorf("option ChipID must be nil or %d bytes", abi.ChipIDStruct0Size)
		default:
	...
	switch product {
	case spb.SevProduct_SEV_PRODUCT_TURIN:
		return fmt.Errorf("option ChipID for Turin must be nil, %d, or %d bytes", abi.ChipIDStruct1Size, abi.ChipIDReportSize)
	case spb.SevProduct_SEV_PRODUCT_UNKNOWN, spb.SevProduct_SEV_PRODUCT_MILAN, spb.SevProduct_SEV_PRODUCT_GENOA:
		return fmt.Errorf("option ChipID must be nil or %d bytes", abi.ChipIDStruct0Size)
	default:
		return fmt.Errorf("unknown product: %v", product)
